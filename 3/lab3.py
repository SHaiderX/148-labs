...
class Player:
    ...
    def move(self, current: int, min_: int, max_: int, goal: int) -> int:
        '''Lets player move up in the game'''
        raise NotImplementedError

class RandomPlayer(Player):
    '''A player who picks moves randomly
    ...
    def move(self, current: int, min_: int, max_: int, goal: int) -> int:
        '''Chooses random int between min_ and max_'''

        x = randint(min(min_, goal - current), min(max_, goal - current))
        return x
...
if __name__ == '__main__':
    # Uncomment the lines below to check your work using python_ta and doctest.
    import python_ta
