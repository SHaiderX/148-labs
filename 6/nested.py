...
def greater_than_all(obj: Union[int, List], n: int) -> bool:
    ...
    if isinstance(obj, int):
        ...
        all_ = []
        for sublist in obj:
            all_.append(greater_than_all(sublist, n))
...
def nested_list_equal(obj1: Union[int, List], obj2: Union[int, List]) -> bool:
    ...
    if isinstance(obj1, int):
        ...
        for i in range(len(obj1)):
            all_.append(nested_list_equal(obj1[i], obj2[i]))
    ...
    if isinstance(obj, int):
        ...
        for sublist in obj:
            if isinstance(sublist, int):
                all_.extend(duplicate(sublist))
            else:
                all_.append(duplicate(sublist))
        return all_
